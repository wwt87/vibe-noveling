...
- **先定边界再讲故事大纲** — `/novel-plan` 会先确定起始状态和结束目标，再写成故事大纲展示整体逻辑，逐步澄清后产出 `剧情思路卡 + 叙述式可写场景纲要` 的双层大纲，并内置大纲 AI 味检测；确认后生成 HTML 可视化（因果链流程图 + 纵向场景卡片），用户在浏览器中审阅编辑后复制回对话
...
| `/novel-plan`      | 规划下一章             | 先确定起始状态和结束目标，再写成故事大纲，逐步澄清后生成 `剧情思路卡 + 叙述式可写场景纲要` 的双层大纲 + 大纲 AI 味检测 + HTML 可视化审阅 + Opus 正文测试，并内置规划期强制反思 |
...
/novel-bookplan      →  全书架构与卷规划（可选）
    ↓
/novel-plan          →  先确定起始状态和结束目标，再写成故事大纲，逐步澄清后生成双层大纲（剧情思路卡 + 叙述式可写场景纲要）+ 大纲 AI 味检测 + 生成 HTML 可视化（因果链流程图 + 纵向场景卡片）供审阅编辑 + 内置规划期强制反思
    ↓
...
这套工作流现在分成 7 个阶段，核心原则是：设定先落长期记忆，结构先落 `future/`，单章先定边界再讲故事大纲，确认后通过 HTML 可视化审阅编辑再落盘，最后进入正文创作。`booming` 是插在剧情讨论阶段里的可选爆破分支，`fuck-it` 是插在剧情讨论或单章规划阶段里的同终点加戏分支，它们都不是默认主流程节点。
...
10. 澄清后写成双层大纲，包含 `剧情思路卡` 和 `叙述式可写场景纲要`（场景纲要为自然段落，场景目标/触发事件/冲突落地/转折点/落点变化/细节锚点嵌入文中）
11. 执行大纲 AI 味检测（7 项，详见 planning-checks.md 第 4 节），逐场景检查，通过后方可进入确认
12. 正式大纲草稿确认后，再单独确认章节标题
13. 如果标题还没想好，可以先记为”待定”
14. 故事梗概确认后生成 HTML 可视化（`synopsis-view.html`，因果链流程图），用户审阅编辑后复制粘贴回对话，写入 `故事梗概.md`
15. 大纲确认后生成 HTML 可视化（`outline-view.html`，纵向场景卡片），用户审阅编辑后复制粘贴回对话，写入 `大纲.md`
15. 从用户粘贴的内容写入最终的 `故事梗概.md` 和 `大纲.md`
16. 默认继续生成 `上下文.md`，并用 Opus 做一次试写，反推结构问题

这里有几个重要边界：

- 故事大纲是自然段落叙述（3-8 段），通过 HTML 可视化审阅编辑后写入 `故事梗概.md`，不写入最终 `大纲.md`
- HTML 可视化包含因果链流程图（故事梗概）和纵向场景卡片（大纲），所有文本区域可内联编辑，编辑后通过复制按钮将 Markdown 粘贴回对话
